        ...
        results.push({
          ...
          regulation: regulation
            ? {
                ...
                displayValue: regulation.displayValue,
              }
            : null,
        });
      }
...
// Get regulations by type across all municipalities (useful for comparisons)
export const getRegulationsByType = query({
  args: { regulationType: v.string() },
  handler: async (ctx, args) => {
    const regulations = await ctx.db
      .query("regulationUrls")
      .withIndex("by_type", (q) => q.eq("regulationType", args.regulationType as any))
      .collect();

    // Get municipality names for each regulation
    const results = await Promise.all(
      regulations.map(async (reg) => {
        const municipality = await ctx.db.get(reg.municipalityId);
        return {
          municipality: municipality?.name || "Unknown",
          municipalityType: municipality?.type || "unknown",
          url: reg.url,
          status: reg.status,
          displayValue: reg.displayValue,
        };
